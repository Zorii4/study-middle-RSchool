...
interface INormolizedData {
  byId: { [id: string]: IPosts }
  allIds: string[]
}
...
const normolizedData: INormolizedData = {
  byId: {},
  allIds: [],
}

const normalizeData = (unnormalizedData: IPosts[]): INormolizedData => {
  unnormalizedData.forEach((el) => {
    normolizedData.byId = { ...normolizedData.byId, [el.id]: el }
    normolizedData.allIds.push(el.id)
  })
